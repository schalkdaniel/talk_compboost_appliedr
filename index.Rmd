...
cboost = boostSplines(data = mulled_wine_data, target = "mw_consumption", 
  loss = LossQuadratic$new(), learning.rate = 0.005, iterations = 3000L, 
  trace = 600L, n.knots = 10, degree = 3)
```
...
for (feature_name in setdiff(names(mulled_wine_data), target)) {
  if (feature_name %in% c("gender", "country")) {
    cboost$addBaselearner(feature = feature_name, id = "category", 
      bl.factory = BaselearnerPolynomial, degree = 1L, intercept = FALSE)
  } else {
    cboost$addBaselearner(feature = feature_name, id = "spline", 
      bl.factory = BaselearnerPSpline, degree = 3L, n.knots = 10L)
  }
}
```
...
cboost$train(3000L, trace = 1500L)
...
risk_data = data.frame(
  ...
  iter = rep(seq_along(inbag_trace), times = 2L)
)
```
...
cboost = boostSplines(data = mulled_wine_data, target = "mw_consumption", 
  ...
  learning.rate = 0.005, iterations = 3000L, trace = 600L, n.knots = 10, 
  degree = 3)
```
...
-   Complicated stuff like compilation, or again, the conversion between `R` and `C++` is handled automatically
